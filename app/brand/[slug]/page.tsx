...
export default function BrandPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(`/api/products?brand=${encodeURIComponent(slug)}&limit=40`);
      ...
      <main className="bg-white">
        ...
        <section className="bg-background-secondary py-10">
          <div className="container mx-auto px-4">
            ...
            <p className="text-text-secondary text-sm md:text-base mt-1">Discover {title} products.</p>
          </div>
        </section>
